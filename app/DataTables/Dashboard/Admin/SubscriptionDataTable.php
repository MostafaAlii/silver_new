...
class SubscriptionDataTable extends BaseDataTable {
    ...
    public function dataTable($query): EloquentDataTable {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('status', function (Subscription $subscription) {
                return $this->StatusChange($subscription->status,$subscription->status());
            })
            ->rawColumns(['action', 'created_at', 'updated_at','status','admin_id', 'employee_id', 'agent_id', 'company_id', 'status']); 
    }
